...
const LoginPage = () => {
  ...
  const handleLoginSubmit = (e) => {
    e.preventDefault();
    // Implement your login logic here using loginData
      ...
      <div className="bg-white p-8 rounded-lg shadow-lg">
        <div className="mb-4">
          <ul className="flex border-b">
            <li
              className={`mr-1 cursor-pointer ${activeTab === 'login' ? 'border-indigo-500 text-indigo-600' : 'border-gray-300'}`}
              onClick={() => handleTabClick('login')}
            >
              <button
                className={`bg-white inline-block border-l border-t border-r rounded-t py-2 px-4 text-blue-700 font-semibold ${activeTab === 'login' ? 'bg-white' : 'bg-gray-100'}`}
              >
                Login
              </button>
            </li>
            <li
              className={`cursor-pointer ${activeTab === 'register' ? 'border-indigo-500 text-indigo-600' : 'border-gray-300'}`}
              onClick={() => handleTabClick('register')}
            >
              <button
                className={`bg-white inline-block border-l border-t border-r rounded-t py-2 px-4 text-blue-700 font-semibold ${activeTab === 'register' ? 'bg-white' : 'bg-gray-100'}`}
              >
                Register
              </button>
            </li>
